...
import type { SpeciesFilterKind, SpeciesGroupProfile } from './match-rules';
import { resolveSpeciesGroup } from './match-rules';
...
function groupFallback(profile: SpeciesGroupProfile): TackleSelection {
	const matching = presets.filter(
		(p) => !p.targetSpecies && profile.fallbackPresetNames.includes(p.name)
	);
	const preset = matching[Math.floor(Math.random() * matching.length)];
	const tackle = resolvePreset(preset);
	const { strata, castStrength } = tacticalOverride(null, tackle.rod, profile.forcedStrata);
...
export function pickBotTackle(
	...
	const profile = resolveSpeciesGroup(speciesFilterKind);
	...
	if (Math.random() < speciesChance) {
		...
		if (idx !== -1) {
			...
			if (!profile || profile.species.includes(targetName)) {
				const preset = presets.find((p) => p.targetSpecies === targetName);
				if (preset) {
					const tackle = resolvePreset(preset);
					const sp = species.find((s) => s.name === targetName) ?? null;
					const { strata, castStrength } = tacticalOverride(sp, tackle.rod, profile?.forcedStrata);
					return { ...tackle, strata, castStrength };
				}
			}
		}
	...
	if (profile) return groupFallback(profile);
