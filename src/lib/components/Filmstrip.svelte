<script lang="ts">
	...
	$effect(() => {
		if (selected && container) {
			const tile = container.querySelector(`[data-filmstrip-id="${selected}"]`);
			if (tile) {
				tile.scrollIntoView({
					block: 'nearest',
					inline: 'center',
...
>
	{#each items as item (item.id)}
		{#if variant === 'selectable'}
			...
			>
				<div
					class="{tileClass} overflow-hidden rounded-xl border-2 transition-all {selected ===
					item.id
		...
		{:else}
			<div data-filmstrip-id={item.id} class="shrink-0">
				<div class="relative">
					<div class="{tileClass} overflow-hidden rounded-xl border-2 border-white/30 shadow-md">
						{#if item.imageUrl}
							<img src={item.imageUrl} alt={item.label} class="h-full w-full object-cover" />
						{:else}
							<div class="flex h-full w-full items-center justify-center bg-surface/60">
				...
				{#if item.description}
					<p class="mt-1 text-center text-sm text-muted">{item.description}</p>
				{/if}
			</div>
		{/if}
	{/each}
